      ...
      async (response) => {
        ...
        if (
          ...
          const updates = response.data.results.map((result: any) => {
            ...
            if (rfceData?.estado) {
              const estado = String(rfceData.estado).toLowerCase();
              dataToUpdate.estatus =
                estado === 'aceptado' ? 'Aceptado' : 'Rechazado';
            } else if (xmlResponseData?.estado) {
              ...
              // Sin estado explícito: conservar 'Firmado' y consultar por trackId
              dataToUpdate.estatus = 'Firmado';
            } else {
              dataToUpdate.estatus = 'Firmado';
            }

            // Soportar formato alterno: { status, message, dgii }
            if (result.dgii) {
              const estadoAlt = String(result.dgii.estado || '').toLowerCase();
              dataToUpdate.estatus =
                estadoAlt === 'aceptado' ? 'Aceptado' : 'Rechazado';
              const mensajeAlt =
                result.dgii.mensajes?.[0]?.valor || result.message || '';
              if (mensajeAlt) dataToUpdate.mensajeError = mensajeAlt;
            } else if (result.status === 'error') {
              const mensajeAlt = result.message || '';
              if (mensajeAlt) dataToUpdate.mensajeError = mensajeAlt;
              dataToUpdate.estatus = 'Rechazado';
            }

            const documentId = scenario.id;

            if (documentId) {
              const docRef = doc(this.firestore, 'DataFromExcel', documentId);
  ...
  aplicarFiltros(): void {
    ...
    if (this.tipoSeleccionado !== '0') {
      dataFiltrada = dataFiltrada.filter(
        (item) => item.TipoeCF == this.tipoSeleccionado
      );
    }
    ...
    if (this.estadoSeleccionado !== 'Todos') {
      dataFiltrada = dataFiltrada.filter(
        (item) => item.estatus === this.estadoSeleccionado
      );
    }
    ...
    if (this.textoBusqueda.trim() !== '') {
      ...
      dataFiltrada = dataFiltrada.filter(
        (item) => item.ENCF && item.ENCF.toLowerCase().includes(searchTerm)
      );
    }
