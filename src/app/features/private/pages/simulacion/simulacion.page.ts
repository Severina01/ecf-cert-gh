...
export class SimulacionPage implements OnInit {
    ...
    generarComprobantesDesdeBase(baseJson: any[], rncPersonalizado: string): any[] {
        ...
        for (const key in cantidadPorTipo) {
            ...
            const plantilla = baseJson.find(
                (f) => String(f.TipoeCF) === tipoReal && this.matchMonto(key, f.MontoTotal)
            );
        ...
        if (factura46 && factura34s.length > 0) {
            for (const factura34 of factura34s) {
                ...
                factura34.RazonModificacion = factura34.RazonModificacion || 'Error en datos';
            }
        }
    ...
    generarYGuardar() {
        const lista = this.generarComprobantesDesdeBase(this.jsonData, this.empresaID);
    ...
    resetearDatos() {
        ...
        this.encfService.resetearENCFs(this.empresaID, this.empresaNombre).then((response) => {
    ...
    parsearFactura(xmlString: string) {
        ...
        this.ENCF = doc.querySelector('eNCF')?.textContent || '';
        this.RNCEmisor = doc.querySelector('RNCEmisor')?.textContent || '';
        this.RNCComprador = doc.querySelector('RNCComprador')?.textContent || '';
        this.FechaEmision = doc.querySelector('FechaEmision')?.textContent || '';
        this.MontoTotal = doc.querySelector('MontoTotal')?.textContent || '';
        this.FechaFirma = doc.querySelector('FechaHoraFirma')?.textContent || '';
        const firma = doc.querySelector('SignatureValue')?.textContent || '';
        this.CodigoSeguridad = firma.substring(0, 6);
        this.RazonSocialComprador = doc.querySelector('RazonSocialComprador')?.textContent || '';
        this.RazonSocialEmisor = doc.querySelector('RazonSocialEmisor')?.textContent || '';
        this.DireccionEmisor = doc.querySelector('DireccionEmisor')?.textContent || '';
        this.subTotal = doc.querySelector('MontoGravadoTotal')?.textContent || '';
        this.itbis = doc.querySelector('TotalITBIS')?.textContent || '';
        this.DireccionComprador = doc.querySelector('DireccionComprador')?.textContent || '';
        const monto = parseFloat(this.MontoTotal);
        ...
        this.productos = Array.from(doc.querySelectorAll('Item')).map((item: Element) => ({
            ...
            CantidadItem: item.querySelector('CantidadItem')?.textContent || '',
            PrecioUnitarioItem: item.querySelector('PrecioUnitarioItem')?.textContent || '',
            MontoItem: item.querySelector('MontoItem')?.textContent || '',
        }));
    }
    ...
    descargarPDF() {
        ...
        html2canvas(data, {
            scale: 1,        // menor resolución = menos peso
            useCORS: true,
            allowTaint: false,
        }).then((canvas) => {
            const imgData = canvas.toDataURL('image/jpeg', 1); // usa JPEG y baja calidad
            ...
            pdf.addImage(imgData, 'JPEG', marginX, marginY, usableWidth, usableHeight);
    ...
    obtenerDatosEmpresa() {
        ...
        collectionData(q, { idField: 'id' }).subscribe((data) => {
            this.empresaLogo = data[0]["logoURL"];
            this.empresaNombre = data[0]["nombreEmpresa"];
        });
    }

    obtenerEncabezadoComprobante(tipo: number | string, montoTotal: string): string {
        const tipoStr = tipo.toString();

        // Convertir montoTotal a número eliminando comas o espacios si los hay
